...
from models.sentiment_analyser import SentimentModel
...
def analyze(data: TextData):
    ...
    try:
        ...
        result = sentiment.annotate_sentiments(input_csv='./data/reviews.csv', output_csv='./data/reviews_labeled.csv') 
...
def scrape_reviews_endpoint(request: ScrapeRequest):
    ...
    try:
        ...
        reviews_list = scraper.scrape_flipkart_reviews(request.url, 10)

        if not reviews_list:
            raise HTTPException(status_code=404, detail="No reviews were found. Check the URL or the website structure.")
        scraper.save_reviews(reviews_list, './data/reviews.csv')
